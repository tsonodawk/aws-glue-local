...
# ==============================
# print("debug01.5")
# sdf_journal_filter_data.filter(
#     (sdf_journal_filter_data.Ymd >= "2020-06-10")
#     & (sdf_journal_filter_data.Ymd <= "2020-06-13")
#     & (sdf_journal_filter_data.Qty > 0)
# ).show()

sdf_sales_itmcd = sdf_journal_filter_data.filter(
    (sdf_journal_filter_data.Ymd >= "2020-06-10")
    & (sdf_journal_filter_data.Ymd <= "2020-06-13")
    & (sdf_journal_filter_data.Qty > 0)
).select(["itmcd"]).distinct()

# >> debug
# 日付確認
# sdf_sales_itmcd.select(["Ymd"]).distinct().sort("Ymd").show()

# 販売実績のある商品コード一覧を作成
# itemcdをユニークにする
# sdf_sales_itmcd = sdf_sales_itmcd.select(["itmcd"]).distinct()
#.filter(sdf_sales_itmcd.itmcd == "000000000049401073")

# print("debug02")
# sdf_sales_itmcd.show()
# << debug
...
# 過去のデータを作成
# =============================
# TODO:ここで最小販売日付にまとめておくか？
sdf_sales_before_data = sdf_journal_filter_data.filter(
    (sdf_journal_filter_data.Ymd >= "2020-05-23")
    & (sdf_journal_filter_data.Ymd <= "2020-05-25")
    & (sdf_journal_filter_data.Qty > 0)
)

# print("debug03")
# sdf_sales_before_data.show()

# 販売実績がある商品のinner join
sdf_sales_itmcd_before_data = sdf_sales_before_data.join(
    sdf_sales_itmcd,
    sdf_sales_before_data.itmcd == sdf_sales_itmcd.itmcd,
    "inner"
    ).select(sdf_sales_before_data.itmcd, sdf_sales_before_data.Ymd)

# 対処商品の最小販売日付を作成
sdf_sales_itmcd_start_ymd = sdf_sales_itmcd_before_data.groupBy(
        'itmcd'
    ).agg(
        func.min('Ymd').alias('min_ymd')
    )
# 出力
out_gdf_sales_itmcd_start_ymd = s3_write_spark_dataframe_single_file(
    sdf_sales_itmcd_start_ymd,
